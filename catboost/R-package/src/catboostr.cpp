...
EXPORT_FUNCTION CatBoostIsGroupwiseMetric_R(SEXP modelParam) {
    ...
    R_API_BEGIN();
    TFullModelHandle model = reinterpret_cast<TFullModelHandle>(R_ExternalPtrAddr(modelParam));
    LOGICAL(result)[0] = static_cast<int>(IsGroupwiseMetric(model->GetLossFunctionName()));
    UNPROTECT(1);    
    return result;
    R_API_END();
}
...
EXPORT_FUNCTION CatBoostFit_R(SEXP learnPoolParam, SEXP testPoolParam, SEXP fitParamsAsJsonParam) {
    REprintf("jea: CatBoostFit_R with david-cortes Rleaks fixes are in.\n");
...
EXPORT_FUNCTION CatBoostSumModels_R(SEXP modelsParam,
    ...
    CB_ENSURE(TryFromString<ECtrTableMergePolicy>(CHAR(ctrMergePolicyParam_char), mergePolicy),
...
EXPORT_FUNCTION CatBoostGetModelParams_R(SEXP modelParam) {
  ...
  // jea: returning empty string... not good!
  /*
EXPORT_FUNCTION broken_CatBoostGetPlainParams_R(SEXP modelParam) {
    SEXP uw_token = PROTECT(R_MakeUnwindCont());
    R_API_BEGIN();
    TFullModelHandle model = reinterpret_cast<TFullModelHandle>(R_ExternalPtrAddr(modelParam));
  ...
  */
  ...
  // jea: this is the v1.2 version, but R_API_END moved to the end.
